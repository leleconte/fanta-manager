function createLeague(userId,{name,logo,maxParticipants,credits,timer,rules}){
  ...
  const d = loadData();
  if(!d.users.some(u=>u.id===userId)) throw new Error('Utente non trovato nel browser. Effettua nuovamente il login.');
  const league={
...
function joinLeague(userId,joinCode){
  ...
  if(l.members.includes(userId)) throw new Error('Sei già membro di questa lega.');
  if(l.members.length>=l.maxParticipants) throw new Error('La lega è piena.');
